...
phrase = namedtuple("phrase", "english, logprob, probs")
def TM(filename, k):
  sys.stderr.write("Reading translation model from %s...\n" % (filename,))
  tm = {}
  for line in open(filename).readlines():
    (f, e, prob) = line.strip().split(" ||| ")
    values = prob.strip().split()
    p = [float(a) for a in values]
    logprob = sum(p)
    tm.setdefault(tuple(f.split()), []).append(phrase(e, logprob, p))
